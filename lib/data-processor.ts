    ...
    records.forEach(record => {
      ...
      records.forEach(record => {
        ...
        if (isLevel1) {
          ...
          // In geography mode with explicit segment selections, show each segment as a separate bar
          // Otherwise, aggregate child geographies under their parent
          if (hasUserSelectedSegments) {
            // User selected specific segments - show each segment as a bar
            key = record.segment

            // Prevent double-counting: if this segment has an aggregated record, skip leaf records
            if (aggregationLevel === null && segmentAggregatedMap.has(key) && record.is_aggregated === false) {
              return // Skip leaf record, use aggregated one instead
            }
          } else {
  ...
  return years.map(year => {
    ...
    records.forEach(record => {
      ...
      if (viewMode === 'segment-mode') {
        ...
        // In geography-mode, determine whether to group by geography or segment
        // When user explicitly selected segments, show each segment as a separate line
        // Otherwise, show each geography as a separate line
        ...
        if (hasExplicitLevel1Selection) {
          // User selected specific segments - show each segment as a separate line
          // Match record to one of the selected segments
          const matchedSegment = selectedSegmentNames.find(seg => {
            if (record.segment === seg) return true
            const hierarchy = record.segment_hierarchy
            return hierarchy.level_1 === seg || hierarchy.level_2 === seg
          ...
          if (matchedSegment) {
            if (record.is_aggregated && selectedSegmentNames.includes(record.segment)) {
              ...
              if (usedAggregatedRecords.has(matchedSegment)) {
                return // Skip - already have aggregated value
              }
              key = matchedSegment
            }
          } else {
          ...
          // No explicit segment selection - group by geography (original behavior)
          let mappedGeo = record.geography
