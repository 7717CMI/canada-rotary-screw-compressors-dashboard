...
import { filterData, prepareGroupedBarData, getUniqueGeographies, getUniqueSegments, getGeographyProportions } from '@/lib/data-processor'
...
export function GroupedBarChart({ title, height = 400 }: GroupedBarChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    console.log('📊 Chart Data Debug:', {
      ...
      willCallFunction: 'prepareGroupedBarData'
    })
    ...
    // Prepare chart data
    // Always use prepareGroupedBarData which handles all cases:
    // - Level 1/Level 2 aggregation
    // - Stacked bars (geography-mode + multiple segments, or segment-mode + multiple geographies)
    // - Non-stacked bars
    // - Global data mapping
    // Previously prepareIntelligentMultiLevelData was used when effectiveAggregationLevel === null,
    // but it doesn't handle stacked mode in geography-mode (groups all segments under one geography key)
    const prepared = prepareGroupedBarData(filtered, modifiedFilters, byRegionRecords, geographyCountries)
    ...
    console.log('📊 Prepared chart data:', {
      ...
      usingFunction: 'prepareGroupedBarData'
    })
