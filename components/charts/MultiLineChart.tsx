...
import { filterData, prepareLineChartData, getUniqueGeographies, getUniqueSegments, getGeographyProportions } from '@/lib/data-processor'
...
export function MultiLineChart({ title, height = 400 }: MultiLineChartProps) {
  ...
  const chartData = useMemo(() => {
    ...
    // Always use prepareLineChartData which handles all cases:
    // - Level 2 aggregation (parent segments when no segment selected)
    // - Geography-mode with explicit segment selections (each segment as a separate line)
    // - Segment-mode with explicit Level 1 selections
    const prepared = prepareLineChartData(filtered, filters, byRegionRecords, geographyCountries)
    ...
    if (isRegionalSegmentType && advancedSegments.length > 0) {
      ...
      // Determine series based on whether user has explicit segment selections
      const segmentsFromSameType = advancedSegments.filter(
        (seg: any) => seg.type === filters.segmentType
      )
      const hasExplicitSegments = segmentsFromSameType.length > 0

      if (hasExplicitSegments) {
        // User selected specific segments - each segment becomes a line
        // Extract series from prepared data keys (which are now segment names)
        series = extractSeriesFromPreparedData()
      } else {
        // No explicit segments - lines represent geographies
